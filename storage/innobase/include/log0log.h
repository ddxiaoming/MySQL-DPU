...
struct log_t{
    ...
    // 已经从redo log buffer写入到redo log file的redo log的lsn
	lsn_t		flushed_to_disk_lsn;
					/*!< how far we have written the log
					AND flushed to disk */

    /*
     * 正在排队将redo log buffer中的redo log刷到磁盘redo log file的操作数量。
     * 这个值最大为1
     * */
    ...
    // 若当前有正在进行的redo fsync，并且本次请求也是fsync操作，则需要等待上次fsync操作完成
	...
	// 去掉LOG_FIL_HEADER和LOG_BLOCK_HEADER后，实际能存放redolog的大小
	...
	mtr_buf_t*	append_on_checkpoint;
					...
					pointer is not NULL. */
    // 排队进行写入checkpoint信息的操作数量。在进行checkpoint时递增，在checkpoint完毕后递减
	ulint		n_pending_checkpoint_writes;
					/*!< number of currently pending
					checkpoint writes */

	// 每次写checkpoint信息时需要加x锁。由异步io线程释放该x锁
