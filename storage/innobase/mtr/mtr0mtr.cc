...
/** Start a mini-transaction. 初始化MTR的各个状态变量
...
mtr_t::Command::prepare_write()
{
    ...
    // 当前MTR产生的redo大于redo log buffer的一半，就需要先扩容redo log buffer
    ...
    /**
     * 在5.6及之前的版本中，每次crash recovery时都需要打开所有的ibd文件，
     * 如果表的数量非常多时，会非常影响崩溃恢复性能，因此从5.7版本开始，
     * 每次checkpoint后，第一次修改的文件名被记录到redo log中，
     * 这样在重启从检查点恢复时，就只打开那些需要打开的文件即可。
     */
...
/** Append the redo log records to the redo log buffer
 * 将MTR内的rodolog添加到redo log buffer
@param[in] len	number of bytes to write */
void
