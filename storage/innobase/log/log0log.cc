...
log_margin_checkpoint_age(
	ulint	len)
{
    // 计算产生的redo log加上log block header和trailer后，实际占用log buffer的大小
    ...
    /*
     * redo log file所剩余的空间不足以容纳当前产生的redo log
     * 我们需要做一次同步的checkpoint来释放redo log file的空间，此次的checkpoint不会同步刷新脏页
     * 做checkpoint之前先要检查一下，此次checkpoint能不能释放出足够的空间来：
     *  1. 能够释放出足够的空间：直接做一次checkpoint
     *  2. 不能释放出足够的空间：当前线程hang住，等待100s！！！100s之后，再做一次checkpoint
     * */
